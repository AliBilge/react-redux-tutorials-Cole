...
export class App extends React.Component<IAppProps> {
  ...
  public render() {
    return (
      <Grid centered>
        <Grid.Row>
          <h1>React + Redux Example! With "user cart"... (If we can call it that to be honest?)</h1>
        </Grid.Row>
        <Grid.Row>
        ...
        <Grid.Row>
          ...
          <h3>Selected Items List by Item Id:</h3>
          {this.props.selectedItems.map(individualSelectedItem => (
            <div>{`Item Id: ${individualSelectedItem}`}</div>
          ))}
        </Grid.Row>
      </Grid>
    );
